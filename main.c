...
static int GamepadCorePreInit(
	...
	info_gamepad->control_proc = NULL; // NOTE: `control_proc` only needed if we intend to support DEVICE_CORE, DEVICE_ABS_CALIB, DEVICE_ABS_AREA
...
static void GamepadCoreUnInit(
	...
	if (info_gamepad->private) {
		...
		private->info_gamepad = NULL;
		private->info_keyboard->private = NULL;
		private->info_keyboard = NULL;
...
_X_EXPORT struct _InputDriverRec GAMEPAD = {
    ...
    .capabilities = 0 // driver owns the gamepad file descriptor, not the xserver by design
#endif
};
