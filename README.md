...
**Development status**: working Xinput driver for gamepads on linux.
...
### Week 4 Driver Binding Keyboard
...
-**memory handling**: understanding what the xserver does with the gamepad and attached keyboard device is important to know what do when the xserver calls the driver code to uninitialize the driver data. If an error happens during the keyboard setup the code needs to free the virtual memory and ask the xserver to delete the input device via `xf86DeleteInput()`. On the other hand, if the error happens with the gamepad we also need to release the virtual memory but we also need to unbind the address of the virtual memory from the keyboard device also so that the xserver won't crash trying to free it later. That will happen when the user unplugs the gamepad from the computer, that's when the xserver will attempt to remove the keyboard that was attached to the gamepad device. This is important to mention because the `xf86-input-joystick` driver handles this in its own way but does not explain why it does not call free() (that driver uses malloc/free for the private data). That left me wondering why it also did not even try to delete the attached keyboard. So I found out through experimentation by running the xserver with `gdb`. Probably `device_remove()` or similar functions in `config/udev.c` will shows us this behavior and I am guessing that it uses `config_info` option to find all the data related to the gamepad that has been orphaned and needs cleanup. Up to this point the source code is in the state of this commit [b0e69e12](https://github.com/misael-diaz/xf86-input-gamepad/tree/b0e69e12a16f1556afd614bca059ef2f7290fc73).

-**inits gamepad properties**: probes the gamepad via `ioctl` calls to find out the number of axes and buttons from the driver code. This is based on the initial code that I wrote when I began this project. There's still work to do like mapping the buttons to keycodes, enabling the gamepad device, maybe set properties on the xserver for client apps to query them, etc. The xserver still rejects the driver (but this is expected) but doing so does not crashes the server (driver is getting into shape). last commit [8a7c38c0](https://github.com/misael-diaz/xf86-input-gamepad/tree/8a7c38c054b8095e844e87f85e7d3bf263da74a1).

-**adds gamepad valuators**: without valuators that validate the axis values and the screen position the xserver simply ignores the events generated by the device. This is the one feature in the `xf86-input-joystick` that we must preserve for the mentioned reasons. After adding the valuators we have a **working xinput driver for gamepads**. Personally, I don't like the key auto-repeat feature and that's something to be addressed later. Last commit [367799d9](https://github.com/misael-diaz/xf86-input-gamepad/tree/367799d95c46cb4d3c3fad9a2d559db6bc5c6689).
...
```sh
gcc -DDEVBUILD=1 -I/usr/include/pixman-1 -Wall -Wextra -Wformat -Wno-comment -fPIC -shared -O0 -gdwarf-4 -g driver.c -o gamepad.so
```
